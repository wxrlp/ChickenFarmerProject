 ...
 * A bee hive should reload every 240 ticks but if the player is
 * standing on it then it should
 * increase the rate by 3 (so if the player stands on it for the
 * whole time, it will
 * reload in 80 ticks).
...
public class BeeHive extends Npc {
    ...
    private final RepeatingTimer timer =
            new RepeatingTimer(RELOAD_COOLDOWN_TICKS);
    ...
    /**
     * Progress the state of the beehive, updating how it is
    ...
    /**
     * Interact with the beehive to increase spawn rate of bees at
    ...
    public Npc checkAndSpawnBee(ArrayList<Enemy> targets) {
        for (Enemy enemy : targets) {
            if (this.distanceFrom(enemy) < DETECTION_DISTANCE
                ...
                return new GuardBee(
                        this.getX(), this.getY(),
                        enemy); // can only spawn one bee in a frame
            }
        }
        return null;
    }
