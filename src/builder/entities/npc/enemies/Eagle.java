...
/**
 * Represents an Eagle enemy that tracks a target, attacks to steal
 * food, and returns to its spawn
...
public class Eagle extends Enemy implements Expirable {
    ...
    /**
     * Constructs an Eagle enemy at the specified coordinates,
     * tracking the given target.
     *
     * @param x             The x-coordinate of the eagle's spawn
     *                      position.
     * @param y             The y-coordinate of the eagle's spawn
     *                      position.
    ...
    public Eagle(int x, int y, HasPosition trackedTarget) {
        ...
        // derive direction based on where the eagle is and the
        // initial target is
        int direction = 20;
     ...
     *
     * @param timer The FixedTimer to set as the lifespan of the
     *              Eagle.
     */
    @Override
    ...
    /**
     * Processes a single tick for the Eagle, updating its
     * position, lifespan, and interaction with
     * the player.
     *
     * @param engine The state of the engine, including the mouse,
     *               keyboard information and
     *               dimension. Useful for processing keyboard
     *               presses or mouse movement.
     * @param game   The state of the game, including the player
     *               and world. Can be used to query or
    ...
    public void tick(EngineState engine, GameState game) {
        ...
        if ((this.distanceFrom(playerX, playerY) < tileSize)
            ...
            this.setSpeed(4); // the eagle BOOKS it once it has the
            // food
            //      }
        }
        ...
        // Update sprite based on whether eagle is attacking or
        // returning to spawn
        if (attacking) {
        ...
        //  reaches its spawn (e.g. is attacked by a bee) then it
        //  returns the stolen food to
        //  the player.
